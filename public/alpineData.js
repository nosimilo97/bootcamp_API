document.addEventListener("alpine:init", () => {
  Alpine.data("bootcampApi", () => {
    return {
     ...
     prices: {
      ...
      async analyzeSentence() {
        try {
          const res = await fetch(`http://api/word_game?sentence=${this.sentence}`, {
            method: 'GET',
            headers: {
      ...
      async calculateBill() {
        try {
            const response = await fetch('http://api/phonebill/total', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    async init() {
        try {
            const response = await fetch('http://api/phonebill/prices');
            if (response.ok) {
                const data = await response.json();
                this.prices = data;
            }
    ...
    async calculateAirtime() {
      try {
          const response = await fetch('http://api/enough_airtime/enough', {
              method: 'POST',
              headers: {
                  'Content-Type': 'application/json',
              },
